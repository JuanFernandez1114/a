...
app.get('/students/search', function (req, res) {
  ...
  if (!last_name) {
    return res.status(400).send({ "message": "Missing last_name parameter" });
  }
  ...
  glob("students/*.json", null, function (err, files) {
    ...
    // Filter the files based on the last name
    var matchingStudents = files.reduce(function (matches, file) {
      var studentData = require('./' + file); // Assuming the JSON file contains student data
      if (studentData.last_name === last_name) {
        matches.push(studentData);
      }
      return matches;
    }, []);
...
console.log('Webapp:   http://localhost:5678/index2.html')
